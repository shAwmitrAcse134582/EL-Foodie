...
class _ManageFoodItemsPageState extends State<ManageFoodItemsPage> {
  ...
  void initState() {
    ...
    for (String field in widget.fields) {
      ...
      if (widget.defaultValues != null && widget.defaultValues!.containsKey(field)) {
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    widget.fields.forEach((field) {
                      if (field == 'price') {
                        data[field] = double.tryParse(controllers[field]!.text) ?? 0.0;
                      } else {
...
class DataSearchDelegate extends SearchDelegate<String> {
  ...
  final List<String> fields;
  final Function({DocumentSnapshot? doc, required bool isEditing}) showDialogFunction;

  DataSearchDelegate(this.collectionName, this.fields, {required this.showDialogFunction});
      ...
      builder: (context, snapshot) {
        ...
        return SingleChildScrollView(
          ...
          child: DataTable(
            columns: [
              DataColumn(label: Text('ID')),
              ...fields.map((field) => DataColumn(label: Text(field.capitalize()))),
              DataColumn(label: Text('Actions')),
            ],
                ...
                DataCell(Row(
                  children: [
                    ...
                    IconButton(
                      icon: Icon(Icons.delete),
                      onPressed: () {
                        FirebaseFirestore.instance.collection(collectionName).doc(doc.id).delete();
                      },
                    ),
                  ],
                )),
